...
- Hotfix após teste real: a redução de eventos deixou a UI parecendo travada em `downloaded_files=1` quando os próximos arquivos demoravam para baixar/validar. O backend voltou a emitir progresso de download a cada arquivo concluído (`REMOTE_UPDATE_DOWNLOAD_PROGRESS_INTERVAL = 1`) e adicionou um `AtomicBool` compartilhado para cancelar/parar workers quando um download falhar, evitando aguardar a fila inteira depois de erro crítico. Validações do hotfix: `cargo fmt --manifest-path src-tauri/Cargo.toml` e `cargo check --manifest-path src-tauri/Cargo.toml` passaram.
