...
- O update remoto agora segue um fluxo transacional mais próximo de launchers comuns: verifica todos os arquivos primeiro, monta um plano de divergentes, baixa tudo em uma pasta `updates/<game_id>/<timestamp>/staging`, valida o staging completo e só então aplica/substitui os arquivos no diretório do jogo. Se a validação falhar, a etapa de aplicação não começa.
- O manifesto aceita `update.maxConcurrentDownloads` opcional para controlar downloads paralelos na estratégia `remoteManifest`, com limite interno de segurança. O RavenQuest usa `maxConcurrentDownloads: 8`. O `runner.log` registra `remote_update_parallel_downloads=true`, `remote_update_download_workers=...`, `planned_update_files=...`, `remote_update_download_concurrency=...` e `remote_update_staging_dir=...`.
- A UI e o parser de progresso por `runner.log` agora reconhecem fases/status adicionais do update transacional: `planUpdate`, `prepareStagingDir`, `validateStagedFiles`, `applyStagedFiles`, além dos statuses `validating` e `applying`.
- Otimização posterior do update remoto: cada worker de download agora valida CRC/tamanho do arquivo em staging imediatamente após baixar, eliminando uma segunda leitura sequencial completa de todos os arquivos divergentes. A validação de staging passa a registrar `remote_update_staging_validation=completed_during_parallel_download`.
- Para reduzir overhead contra muitos arquivos pequenos, o update remoto não escreve mais `download_attempt`/sucesso no `runner.log` para cada arquivo baixado com sucesso; mantém logs detalhados de falhas/retries e checkpoints agregados de download com `remote_update_download_validation=parallel_worker`. Eventos de download/aplicação também foram reduzidos para intervalos, e o buffer de CRC subiu para 1 MiB.
- Correção posterior do hotfix de progresso/cancelamento: cancelar no primeiro erro fez o update parecer baixar só 1 arquivo e reduziu o comportamento esperado de launcher normal. O pool de downloads agora usa concorrência real via índice atômico compartilhado entre workers, respeitando `update.maxConcurrentDownloads` (RavenQuest = 8), sem `Mutex<Receiver>` serializando a busca de tarefas. A UI segue recebendo progresso a cada arquivo concluído, enquanto falhas são acumuladas e reportadas ao final para não abortar toda a fila no primeiro arquivo problemático.
- Ajuste posterior de visibilidade do fallback por `runner.log`: os checkpoints agregados do log passaram de 1000 para 100 arquivos, permitindo que a UI mostre avanço mais granular quando a fonte do progresso for `runner.log`. A etapa de aplicação (`applying`) também grava checkpoints no log a cada 50 arquivos, além dos eventos Tauri, para não parecer parada ao aplicar em lotes.
...
- Validações executadas: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `npm run build` e `cargo check --manifest-path src-tauri/Cargo.toml` passaram, incluindo o ajuste que mescla `binary` + `files` no update remoto, o progresso visual/não-bloqueante do update, a instrumentação visual por etapas, o fluxo transacional/paralelo com staging e a validação de staging dentro dos workers paralelos.
