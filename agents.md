...
- Hotfix de progresso/cancelamento: após teste real parecer travado no primeiro arquivo baixado, o frontend voltou a receber evento de progresso a cada arquivo concluído durante download (`REMOTE_UPDATE_DOWNLOAD_PROGRESS_INTERVAL = 1`). O pool de workers também ganhou sinal de cancelamento compartilhado para parar os demais workers assim que uma falha crítica for detectada, evitando ficar aguardando downloads/retries pendentes depois do primeiro erro.
