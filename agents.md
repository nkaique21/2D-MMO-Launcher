...
- Validações executadas: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `npm run build` e `cargo check --manifest-path src-tauri/Cargo.toml` passaram, incluindo o ajuste que mescla `binary` + `files` no update remoto e o progresso visual/não-bloqueante do update.
