...
- O RavenQuest agora declara `update.strategy: "externalLauncher"`, executando `launcher.exe` via Wine no prefixo Proton (`compatPrefix: "proton"`) para permitir atualizar os arquivos pelo launcher oficial sem alterar o fluxo de jogo via BattlEye.
...
- O backend expõe `run_game_update`, que usa a configuração `update` do manifesto, registra `action=run_game_update` no `runner.log` e inicia o updater externo com o runner/env configurados.
- O frontend exibe a ação secundária `Atualizar pelo launcher oficial` somente para jogos instalados cujo manifesto declara `update.strategy: "externalLauncher"`.
...
- Testar a ação `Atualizar pelo launcher oficial` no RavenQuest instalado e conferir no `runner.log` `action=run_game_update`, `update_strategy=externalLauncher`, `update_process_started=true` e o comando apontando para `launcher.exe`.
